...
このため、コサイン類似度は-1から1の範囲の値を持ち、1に近いほど2つのベクトルが同じ方向を向いていることを示します。ちなみに90°なら0、真反対なら-1です。
...
```
まずこれは[疎ベクトル](文書検索つくった話/SparseVector.md)を用いてメモリ効率を大幅に向上させています。  
...
### IDFベクトル構造体
IDFベクトル構造体は、コーパス全体の単語の珍しさを表す構造体です。  
そのため、コーパスに属する文書が1つでも変化すればIDFベクトルも変化します。  
また、IDFベクトルはTFベクトルのように疎なベクトルではなく、すべての単語に対してIDF値を保持する必要があるため、密なベクトルとして実装されます。  
このため以下のような実装になります。  
```rust
...
```
以下に各フィールドの役割を説明します。
- `idf_vec`: コーパス全体の単語のIDF値を保持するベクトルです。これはコーパス全体の単語の珍しさを表すベクトルになります。
- `latest_entropy`: このIDFの世代を表すフィールドです。コーパスが変化するたびにIDFも変化するため、この構造体はキャッシュとしての扱いになります。そこで最新のIDFかを確認するための値になります。(`generation`のほうが一般的)
- `doc_num`: コーパス全体の文書数を表すフィールドです。これはIDFの計算に必要な値になります。

この構造体へのメソッド実装は内部のidf_vecがそもそもVecなのでpublicにしておしまいです。

### TFベクトル, IDFベクトルをまとめた抽象構造体
TF-IDF-Vectorizer構造体は、TFベクトルとIDFベクトルをまとめた抽象構造体です。
```rust
/// TF-IDF Vectorizer
///
...
以下に各フィールドの役割を説明します。
- `documents`: 文書のTFベクトルを保持するマップです。キーは文書の識別子で、値は文書のTFベクトルになります。またこのマップは順序つきハッシュマップで、全走高速化のため特別に実装してます。
- `term_dim_rev_index`: 単語のインデックスと単語を対応させるための逆インデックスです。キーは単語で、値はその単語が出現する次元のリストになります。
- `corpus_ref`: コーパスへの参照です。これはコーパスの情報を取得するために使用されます。
